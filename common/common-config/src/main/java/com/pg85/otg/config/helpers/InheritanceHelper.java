...
public class InheritanceHelper
{
     ...
     */
    public static final <T, C extends ConfigFunction<T>> List<C> mergeLists(Collection<? extends C> childList, Collection<? extends C> parentList, ILogger logger)
    {
        List<C> returnList = new ArrayList<C>(childList);
        for (C parentFunction : parentList)
        {
            if (!hasAnalogousFunction(parentFunction, childList, logger))
            {
                returnList.add(parentFunction);
            }
        }
     ...
     * otherwise.
     */
    private static final <T, C extends ConfigFunction<T>> boolean hasAnalogousFunction(C function, Collection<? extends C> list, ILogger logger)
    {
        for (C toCheck : list)
        {
            if (function.isAnalogousTo(toCheck, logger))
            {
                return true;
            }
        }
