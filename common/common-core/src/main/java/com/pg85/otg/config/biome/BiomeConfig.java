...
public class BiomeConfig extends BiomeConfigBase
{
	...
	public BiomeConfig(String biomeName, BiomeConfigStub biomeConfigStub, Path presetFolder, SettingsMap settings,
	...
	public SaplingResource getCustomSaplingGen(LocalMaterialData materialData, boolean wideTrunk)
	{
		...
		if (wideTrunk)
		{
			...
			// return
			// customBigSaplingGrowers.get(materialData.withBlockData(materialData.getBlockData()
			// % 8));
		}
		return customSaplingGrowers.get(materialData);
		// return
		// customSaplingGrowers.get(materialData.withBlockData(materialData.getBlockData()
		// % 8));
	}
	...
	private void readHeightSettings(SettingsMap settings, double[] heightMatrix, Setting<double[]> setting,
			double[] defaultValue, ILogger logger)
	{
	...
	private void readResourceSettings(SettingsMap settings, IConfigFunctionProvider biomeResourcesManager,
			ILogger logger, IMaterialReader materialReader)
	{
		// Disable resourceinheritance for saplings
		List<ConfigFunction<IBiomeConfig>> resources = new ArrayList<>(
				settings.getConfigFunctions(this, biomeResourcesManager, logger, materialReader));
				...
				if (res instanceof SaplingResource)
				{
					...
					if (sapling.saplingType == SaplingType.Custom)
					{
						...
						} catch (NullPointerException e)
						{
							...
							{
								logger.log(LogLevel.ERROR, LogCategory.CONFIGS,
										"Unrecognized sapling type in biome " + this.getName());
							}
						}
					} else
					{
	...
	protected void writeConfigSettings(SettingsMap writer)
	{
		...
		writer.putSetting(SurfaceGeneratorSetting.SURFACE_AND_GROUND_CONTROL, this.surfaceAndGroundControl,
				...
				"Example: " + SurfaceGeneratorSetting.SURFACE_AND_GROUND_CONTROL
						+ ": STONE,STONE,-0.8,GRAVEL,STONE,0.0,DIRT,DIRT,10.0",
	...
	protected void validateAndCorrectSettings(Path settingsDir, ILogger logger)
	{
		...
		this.volatility1 = this.volatilityRaw1 < 0.0D ? 1.0D / (Math.abs(this.volatilityRaw1) + 1.0D)
				: this.volatilityRaw1 + 1.0D;
		this.volatility2 = this.volatilityRaw2 < 0.0D ? 1.0D / (Math.abs(this.volatilityRaw2) + 1.0D)
				: this.volatilityRaw2 + 1.0D;
