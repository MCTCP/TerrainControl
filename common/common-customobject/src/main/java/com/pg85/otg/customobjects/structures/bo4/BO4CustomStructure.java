...
import com.pg85.otg.util.bo3.BO3Enums.SpawnHeightEnum;
...
{	
    ...
    public BO4CustomStructure(long worldSeed, BO4CustomStructureCoordinate start, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
        ...
        if (!(start.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker) instanceof StructuredCustomObject))
        {
    ...
    public BO4CustomStructure(CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, BO4CustomStructureCoordinate start, boolean isStructureAtSpawn, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingPopulated, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
		...
		if(!doStartChunkBlockChecks(worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
		{
			return;
		}
		...
		try
		{
	    	BO4Config bo4Config = ((BO4)this.start.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
			calculateBranches(bo4Config, false, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
		...
		for(Entry<ChunkCoordinate, Stack<BO4CustomStructureCoordinate>> chunkCoordSet : objectsToSpawn.entrySet())
		{
			if(chunkCoordSet.getValue() != null)
			{
				...
				{
					structureInfo += customObjectCoord.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker).getName() + ":" + customObjectCoord.getRotation() + ", ";
				}
		...
		for(Entry<ChunkCoordinate, Stack<BO4CustomStructureCoordinate>> chunkCoordSet : objectsToSpawn.entrySet())
		{
			if(chunkCoordSet.getValue() != null)
			{
	        	...
	        	{
	        		BO4Config objectConfig = ((BO4)coordObject.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
	        		if(objectConfig.replacesBO3Branches.size() > 0)
	        		{
	        			for(String BO3ToReplace : objectConfig.replacesBO3Branches)
	        			{
	        				...
	        				{
	        					if(((BO4)coordObjectToReplace.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getName().equals(BO3ToReplace))
	        					{
	        						if(checkCollision(coordObject, coordObjectToReplace, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	        						{
		...
		smoothingAreaManager.calculateSmoothingAreas(objectsToSpawn, (BO4CustomStructureCoordinate)this.start, worldGenRegion, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
		...
		if(objectsToSpawn.size() > 0)
		{
			...
			if(spawnLog)
			{
				...
				logger.log(LogMarker.INFO, this.start.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker).getName() + " " + totalBO3sSpawned + " object(s) plotted in " + (System.currentTimeMillis() - startTime) + " Ms and " + Cycle + " cycle(s), " + (branchesTried + 1) + " object(s) tried.");
			}
		}
    }
    ...
    private boolean doStartChunkBlockChecks(IWorldGenRegion worldGenRegion, ChunkCoordinate chunkBeingPopulated, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
    	if(!startChunkBlockChecksDone)
    	{
	    	...
	    	BO4Config config = ((BO4)this.start.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
    ...
    public Object[] getMinimumSize(CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker) throws InvalidConfigException
    {
    	...
    	calculateBranches(bo4Config, true, structureCache, worldGenRegion, null, null, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
    ...
    private void calculateBranches(BO4Config startBO4Config, boolean minimumSize, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingPopulated, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker) throws InvalidConfigException
    {
    	if(spawnLog)
    	{
	    	...
	    	logger.log(LogMarker.INFO, "");
	    	logger.log(LogMarker.INFO, "-------- CalculateBranches " + this.start.bo3Name + sminimumSize +" --------");
    	}
        ...
        {
        	logger.log(LogMarker.INFO, "");
        	logger.log(LogMarker.INFO, "---- Cycle 0 ----");
        	logger.log(LogMarker.INFO, "Plotted X" + branchData.chunkCoordinate.getChunkX() + " Z" + branchData.chunkCoordinate.getChunkZ() + " - " + branchData.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker).getName());
        }

        addToCaches(branchData, ((BO4)branchData.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)));       
    	...
    	while(!processingDone)
    	{
    		...
    		traverseAndSpawnChildBranches(startBO4Config, branchData, minimumSize, true, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
			...
			traverseAndSpawnChildBranches(startBO4Config, branchData, minimumSize, false, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
            ...
            if(processingDone && !canOverrideBranchesSpawned)
            {
	            ...
	            for(BranchDataItem branchDataItem3 : AllBranchesBranchData)
	            {
	            	for(BranchDataItem childBranch : branchDataItem3.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	            	{
	            		if(
            				!childBranch.branch.isRequiredBranch &&
            				((BO4)childBranch.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().canOverride
        				)
	            		{
    		...
    		if(branchData.cannotSpawn)
    		{
    			if(minimumSize)
    			{
    				if(spawnLog)
    				{
    					logger.log(LogMarker.WARN, "Error: Branching BO4 " + this.start.bo3Name + " could not be spawned in minimum configuration (isRequiredBranch branches only).");
    				}
    ...
    private void traverseAndSpawnChildBranches(BO4Config startBO4Config, BranchDataItem branchData, boolean minimumSize, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingPopulated, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
    	...
    	{
    		addBranches(startBO4Config, branchData, minimumSize, false, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
    	} else {
    		if(!branchData.cannotSpawn)
    		{
    			for(BranchDataItem branchDataItem2 : branchData.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
    			{
    				...
    				{
    					traverseAndSpawnChildBranches(startBO4Config, branchDataItem2, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
    				}
    			}
    		}
    	}
    ...
    private void addBranches(BO4Config startBO4Config, BranchDataItem branchDataItem, boolean minimumSize, boolean traverseOnlySpawnedChildren, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingPopulated, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
    	...
    	{
	    	for(BranchDataItem branchDataItem3 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	    	{
	    		if(
    				(
						...
						((BO4)branchDataItem3.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().canOverride &&
    	...
    	{    		
	        for(BranchDataItem childBranchDataItem : branchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	        {        	
	        	if(!AllBranchesBranchDataHash.contains(childBranchDataItem.branchNumber) && !childBranchDataItem.spawnDelayed)
	        	{
        			...
        			BO4 bo3 = ((BO4)childBranchDataItem.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker));
        			...
        			if(bo3 == null || bo3.isInvalidConfig)
        			{
		        		...
		        		if(bo3 == null)
		        		{
		        			if(spawnLog)
		        			{
		        				logger.log(LogMarker.WARN, "Error: Could not find BO3 file: " + childBranchDataItem.branch.bo3Name + ".BO3 which is a branch of " + branchDataItem.branch.bo3Name + ".BO3");
		        			}
		        		}
        			}
	        		...
	        		// Do spawn checks
	        		int smoothRadius = startBO4Config.overrideChildSettings && bo3.getConfig().overrideChildSettings ? startBO4Config.smoothRadius : bo3.getConfig().smoothRadius;
	        		...
	        		if(canSpawn)
	        		{
	        			...
	        			if(!minimumSize && bo3.getConfig().mustBeInsideWorldBorders && !worldGenRegion.isInsideWorldBorder(childBranchDataItem.chunkCoordinate))
	        			{
	        				chunkIsIneligible = true;
	        				canSpawn = false;
	        			}
	        			...
	        			{
	        				canSpawn = checkMustBeBelowOther(childBranchDataItem, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
	        				if(!canSpawn)
	        				{
	        			...
	        			{	        				
	        				canSpawn = checkMustBeInside(childBranchDataItem, bo3, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
	        				if(!canSpawn)
	        				{
	        			...
	        			{
	        				canSpawn = checkCannotBeInside(childBranchDataItem, bo3, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
	        				if(!canSpawn)
	        				{
	        		    ...
	        		    {
	        	    		if(!checkYBounds(worldGenRegion.getWorldConfig().getBedrockDisabled(), childBranchDataItem.branch, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	        	    		{
		    					canSpawn = false;
        						chunkIsIneligible = true;
	        	    		}
	        			...
	        			if(canSpawn)
	        			{
	    					...
	    					collidingBranches = checkSpawnRequirementsAndCollisions(startBO4Config, structureCache, childBranchDataItem, minimumSize, worldGenRegion, targetBiomes, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
		        	...
		        	if(canSpawn)
		        	{
	        	    	...
	        	    	if(childBranchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker).size() == 0)
	        	    	{
	        	    		childBranchDataItem.doneSpawning = true;
	        	    	}

	        	    	// Mark any branches in the same branch group so they wont try to spawn
		        		for(BranchDataItem childBranchDataItem2 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
		        		{
		        			if(
        				...
        				)
		        		{
		        			if(spawnLog)
		        			{
		        				logger.log(LogMarker.INFO, "Plotting all required child branches that are not in a branch group with optional branches.");
		        			}

		        			spawningRequiredChildrenForOptionalBranch = true;
        					currentSpawningRequiredChildrenForOptionalBranch = childBranchDataItem;
			        		traverseAndSpawnChildBranches(startBO4Config, childBranchDataItem, minimumSize, true, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
		        			...
		        			if(spawnLog)
		        			{
		        				logger.log(LogMarker.INFO, "Done spawning required children for optional branch X" + childBranchDataItem.chunkCoordinate.getChunkX() + " Z" + childBranchDataItem.chunkCoordinate.getChunkZ() + (minimumSize ? "" : " Y" + (childBranchDataItem.branch.getY())) + " " +  childBranchDataItem.branch.bo3Name + ":" + childBranchDataItem.branch.getRotation());
		        			}
		        		}
		        	...
		        	if(!canSpawn)
		        	{
		        		if(!childBranchDataItem.doneSpawning && !childBranchDataItem.cannotSpawn)
		        		{
			        		...
			        		if((!wasntBelowOther || !spawnedBranchLastCycle) && branchGroupFailedSpawning)
			        		{
		            			...
		            			rollBackBranch(startBO4Config, branchDataItem, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
		            			bBreak = true;
			        		} else {
				        		...
				        		for(BranchDataItem childBranchDataItem2 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
				        		{
				        			if(!wasntBelowOther || !spawnedBranchLastCycle)
				        			{
				        				BO4Config childBranchDataItem2Config = ((BO4)childBranchDataItem2.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
					        			if(
			        					...
			        					)
					        			{
							        		...
							        		if(childBranchDataItem2.branch.isRequiredBranch)
							        		{
							        	    	...
							        	    	// Check if there are any more branches in this group that haven't tried to spawn yet.
							        			// *At this point, all optional branches should have had a chance to spawn.
								        		for(BranchDataItem childBranchDataItem3 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
								        		{
								        			if(
    		...
    		)
	        {
	        	for(BranchDataItem childBranchDataItem : branchDataItem.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	        	{
	        		if(AllBranchesBranchDataHash.contains(childBranchDataItem.branchNumber))
	        		{
						if(
							(
								...
								(
									...
									!((BO4)childBranchDataItem.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().canOverride
								)
							) &&
						...
						{
							traverseAndSpawnChildBranches(startBO4Config, childBranchDataItem, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
						}
	        		}
	        	}
	        }
		...
		if(AllBranchesBranchDataByChunk.containsKey(childBranchDataItem.chunkCoordinate))
		{
			for(BranchDataItem branchDataItem2 : AllBranchesBranchDataByChunk.get(childBranchDataItem.chunkCoordinate))
			{
				if(
					...
					!((BO4) branchDataItem2.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().canOverride &&
		...
		if(AllBranchesBranchDataByChunk.containsKey(childBranchDataItem.chunkCoordinate))
		{
			...
			for(String cantBeInsideBO3 : bo3.getConfig().cannotBeInsideBranches)
			{
    			for(BranchDataItem branchDataItem3 : branchDataInChunk)
				{
					...
					{
						for(String branchName : ((BO4)branchDataItem3.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().getInheritedBO3s()) // getInheritedBO3s also contains this BO3
						{
							if(branchName.equals(cantBeInsideBO3))
							{
   	    						if(checkCollision(childBranchDataItem.branch, branchDataItem3.branch, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
   	    						{
   	     	        				if(spawnLog)
   	    	        				{
   	     	        					logger.log(LogMarker.INFO, "CannotBeInside branch " + childBranchDataItem.branch.bo3Name + " was blocked by " + branchDataItem3.branch.bo3Name);
   	    	        				}
	...
	private boolean checkMustBeInside(BranchDataItem childBranchDataItem, BO4 bo3, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
	{
	...
	private void rollBackBranch(BO4Config startBO4Config, BranchDataItem branchData, boolean minimumSize, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingPopulated, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
    	...
    	deleteBranchChildren(startBO4Config, branchData, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
    	...
    	if(AllBranchesBranchDataHash.contains(branchData.branchNumber))
    	{
    		if(spawnLog)
    		{
	    		...
	    		logger.log(LogMarker.INFO, "Deleted X" + branchData.branch.getChunkX() + " Z" + branchData.branch.getChunkZ() + " Y" + branchData.branch.getY() + " " + branchData.branch.bo3Name + ":" + branchData.branch.getRotation()  + (branchData.branch.isRequiredBranch ? " required" : " optional") + " cycle " + Cycle + allParentsString);
    		}

    		removeFromCaches(branchData, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);    	
    	}

    	BO4Config branchDataConfig = ((BO4)branchData.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
    		...
    		if(branchDataByChunk != null)
    		{
	    		...
	    		for(BranchDataItem branchDataItem2 : allBranchesBranchData2)
	    		{
	    			if(AllBranchesBranchDataHash.contains(branchDataItem2.branchNumber))
	    			{
		    			...
		    			{
			    			if(((BO4)branchDataItem2.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().mustBeBelowOther && branchDataItem2.chunkCoordinate.equals(branchData.chunkCoordinate))
			    			{
		    					...
		    					{
			    					BO4Config branchDataItem3Config = ((BO4)branchDataItem3.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
			    					if(
			    				...
			    				if(!branchAboveFound)
			    				{
			    					rollBackBranch(startBO4Config, branchDataItem2, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
			    				}
			    			}
		    			}
	    			}
		...
		if(branchDataByChunk != null)
		{
	    	...
	    	for(BranchDataItem branchDataItem2 : allBranchesBranchData2)
	    	{
	    		if(AllBranchesBranchDataHash.contains(branchDataItem2.branchNumber))
	    		{
		    		if(branchDataItem2 != branchData)
					{
		    			BO4 branchDataItem2BO4 = ((BO4)branchDataItem2.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker));
						...
						)
		    			{
							...
							if(currentBO3Found) 
							{
	    	    				...
	    	    				if(!checkMustBeInside(branchDataItem2, branchDataItem2BO4, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	    	    				{
	    	    					rollBackBranch(startBO4Config, branchDataItem2, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
	    	    				}
							}
		...
		if(branchData.parent != null && !branchData.parent.isBeingRolledBack)
		{
    		if(branchData.branch.isRequiredBranch)
    		{
    			...
    			rollBackBranch(startBO4Config, branchData.parent, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
    		} else {
        		...
        		for (BranchDataItem branchDataItem2 : branchData.parent.getChildren(false, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
        		{
				...
				)
    			{
        			...
        			if(!spawningRequiredChildrenForOptionalBranch)
        			{
        				if(spawningRequiredBranchesOnly)
        				{
            				...
            				addBranches(startBO4Config, branchData.parent, minimumSize, false, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
        				} else {
            				...
            				addBranches(startBO4Config, branchData.parent, minimumSize, true, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
        				}
        			} else {
    ...
    {
    	// Remove all children of this branch from AllBranchesBranchData
    	Stack<BranchDataItem> children = branchData.getChildren(true, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
        for(BranchDataItem branchDataItem : children)
        {
        	...
        	if(branchDataItem.getChildren(true, worldGenRegion, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker).size() > 0)
        	{
    			deleteBranchChildren(startBO4Config, branchDataItem, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingPopulated, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);
        	}
        	if(AllBranchesBranchDataHash.contains(branchDataItem.branchNumber))
        	{
        		if(spawnLog)
        		{
	        		...
	        		logger.log(LogMarker.INFO, "Deleted X" + branchDataItem.branch.getChunkX() + " Z" + branchDataItem.branch.getChunkZ() + " Y" + branchDataItem.branch.getY() + " " + branchDataItem.branch.bo3Name + ":" + branchDataItem.branch.getRotation() + (branchDataItem.branch.isRequiredBranch ? " required" : " optional") + " cycle " + Cycle + allParentsString);
        		}

        		removeFromCaches(branchDataItem, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker);

        		BO4Config branchDataItemConfig = ((BO4)branchDataItem.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig();
	        		...
	        		if(branchDataByChunk != null)
	        		{
		        		...
		        		for(BranchDataItem branchDataItem2 : allBranchesBranchData2)
		        		{
		        			if(AllBranchesBranchDataHash.contains(branchDataItem2.branchNumber))
		        			{
			        			...
			        			{
			    	    			if(((BO4)branchDataItem2.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().mustBeBelowOther && branchDataItem2.chunkCoordinate.equals(branchDataItem.chunkCoordinate))
			    	    			{
			    	    				boolean branchAboveFound = false;
			    	    				for(BranchDataItem branchDataItem3 : AllBranchesBranchDataByChunk.get(branchDataItem2.chunkCoordinate))
		    	    					{
	...
	private void removeFromCaches(BranchDataItem branchDataItem, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
	{		
	...
	// Returns null if the branch cannot spawn in the given biome or if there's another BO4 structure in the chunk, otherwise returns colliding branches. 
	// CanOverride branches never collide with other branches, but may be unable to spawn if there's not enough space for smoothing areas.
    private Stack<BranchDataItem> checkSpawnRequirementsAndCollisions(BO4Config startBO4Config, CustomStructureCache structureCache, BranchDataItem branchData, boolean minimumSize, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
    	...
    	if(!minimumSize)
    	{
	    	...
	    	if(smoothRadius == -1 || ((BO4)coordObject.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().smoothRadius == -1)
	    	{
	    		smoothRadius = 0;
	    	}
	    	if(smoothRadius > 0)
        ...
        if(!((BO4) coordObject.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().canOverride)
        {
	        ...
	        if(AllBranchesBranchDataByChunk.containsKey(branchData.chunkCoordinate))
	        {
		        ...
		        {
		        	if(branchData.chunkCoordinate.equals(existingBranchData.chunkCoordinate) && !((BO4)existingBranchData.branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().canOverride)
		        	{
	        ...
	        if (existingBranches.size() > 0)
	        {
	        	...
	        	{
	        		if(checkCollision(coordObject, cachedBranch.branch, otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker))
	        		{
    ...
    private boolean checkYBounds(boolean disableBedrock, CustomStructureCoordinate branchData1Branch, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
        int startY = branchData1Branch.getY() + ((BO4)branchData1Branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker)).getConfig().getminY();
        return disableBedrock ? startY >= 0 : startY > 0;
    }
    
    private boolean checkCollision(CustomStructureCoordinate branchData1Branch, CustomStructureCoordinate branchData2Branch, Path otgRootFolder, boolean spawnLog, ILogger logger, CustomObjectManager customObjectManager, IPresetNameProvider presetNameProvider, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
    {
    	BO4 branch1Object = ((BO4)branchData1Branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker));
    	BO4 branch2Object = ((BO4)branchData2Branch.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker));
    ...
    {   	
        ...
        if (objectsInChunk != null)
        {
            ...
            for (BO4CustomStructureCoordinate coordObject : objectsInChunk)
            {
                ...
                BO4 bo4 = ((BO4)coordObject.getObject(otgRootFolder, spawnLog, logger, customObjectManager, presetNameProvider, materialReader, manager, modLoadedChecker));
