...
import com.khorn.terraincontrol.configuration.io.BracketSettingsReaderOTGPlus;
...
public class CustomObjectCollection
{
    ...
    {    
    	...
    	if(file.isFile())
    	{
            ...
            if (index != -1)
            {
                ...
                // Get the object
                CustomObjectLoader loader = TerrainControl.getCustomObjectManager().getObjectLoaders().get(objectType.toLowerCase());
                if (loader != null)
                {
    		...
    		TerrainControl.log(LogMarker.INFO, "Given path does not exist: " + file.getAbsolutePath());
    ...
    {   	
    	//TerrainControl.log(LogMarker.INFO, "getObjectByName " + worldName != null ? worldName : "");   	
    	...
    	{
    		//TerrainControl.log(LogMarker.INFO, "UseWorld is not used by OTG, skipping it.");
    		return null;
    	}
    	else if(name.equalsIgnoreCase("UseBiome") || name.equalsIgnoreCase("UseBiomeAll"))
    	{
    		//TerrainControl.log(LogMarker.INFO, "UseBiome is not used by OTG, skipping it.");
    		return null;
    	}
    	...
    	{   		
    		...
    		if(new File(TerrainControl.getEngine().getTCDataFolder() + File.separator + "GlobalObjects").exists())
    		{
    			indexAllCustomObjectFilesInDir(new File(TerrainControl.getEngine().getTCDataFolder() + File.separator + "GlobalObjects"), CustomObjectFilesGlobalObjects);
    		}
    	...
    	if(!CustomObjectFilesPerWorld.containsKey(worldName))
    	{
			...
			if(worldName != null && new File(TerrainControl.getEngine().getTCDataFolder() + File.separator + "worlds" + File.separator + worldName + File.separator + "WorldObjects").exists())
			{
				indexAllCustomObjectFilesInDir(new File(TerrainControl.getEngine().getTCDataFolder() + File.separator + "worlds" + File.separator + worldName + File.separator + "WorldObjects"), worldCustomObjectFiles);
			}
    	}    
    		...
    		if(worldCustomObjectFiles != null)
    		{
    			...
    			if(searchForFile != null)
    			{
    				...
    				if(object != null)
    				{
    	        		...
    	        		TerrainControl.log(LogMarker.ERROR, "Could not load BO2/BO3, it probably contains errors: " + searchForFile);
    	        		return null;
    				}
    			}
    		}
    	...
    	TerrainControl.log(LogMarker.ERROR, "Could not find BO2/BO3 " + name + " in GlobalObjects " + (worldName != null ? "and WorldObjects" : "") + " directory " + (worldName != null ? "for world " + worldName : "") + ".");
    		...
    		if(searchDir.isDirectory())
    		{
	    			...
	    			if(fileInDir.isDirectory())
	    			{
	    				...
	    				if(fileInDir.getName().toLowerCase().endsWith(".bo3") || fileInDir.getName().toLowerCase().endsWith(".bo2"))
	    				{
		    				if(!customObjectFiles.containsKey(fileInDir.getName().toLowerCase().replace(".bo3", "").replace(".bo2", "")))
		    				{
		    					...
		    					{
		    						TerrainControl.log(LogMarker.WARN, "Duplicate file found: " + fileInDir.getName() + ".");
		    					}
		    				}
	    				}
	    			}
    			...
    			if(searchDir.getName().toLowerCase().endsWith(".bo3") || searchDir.getName().toLowerCase().endsWith(".bo2"))
    			{
	    			if(!customObjectFiles.containsKey(searchDir.getName().toLowerCase().replace(".bo3", "").replace(".bo2", "")))
					{
						...
						if(TerrainControl.getPluginConfig().SpawnLog)
						{
							TerrainControl.log(LogMarker.WARN, "Duplicate file found: " + searchDir.getName() + ".");
						}
					}
    			}
    		}
