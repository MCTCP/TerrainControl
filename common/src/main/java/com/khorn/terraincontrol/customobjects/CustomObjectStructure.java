...
public class CustomObjectStructure
{
    ...
    {    	
    	if(!startChunkBlockChecksDone)
    	{
	    	...
	    	//TerrainControl.log(LogMarker.INFO, "DoStartChunkBlockChecks");
			...
			if(((BO3)Start.getObject()).getSettings().spawnHeight == SpawnHeightEnum.highestBlock || ((BO3)Start.getObject()).getSettings().spawnHeight == SpawnHeightEnum.highestSolidBlock)
			{
				if(((BO3)Start.getObject()).getSettings().SpawnAtWaterLevel)
				{
					...
					// TerrainControl.log(LogMarker.INFO, "Request height for chunk X" + ChunkCoordinate.fromBlockCoords(Start.getX(), Start.getZ()).getChunkX() + " Z" + ChunkCoordinate.fromBlockCoords(Start.getX(), Start.getZ()).getChunkZ());
					...
					if(highestBlock < 1)
					{
						//TerrainControl.log(LogMarker.INFO, "Structure " + Start.BO3Name + " could not be plotted at Y < 1. If you are creating empty chunks intentionally (for a sky world for instance) then make sure you don't use the highestBlock setting for your BO3's");
			...
			if(startY < TerrainControl.WORLD_DEPTH || startY >= TerrainControl.WORLD_HEIGHT)
			{
				return false;
			}
			...
			//TerrainControl.log(LogMarker.INFO, "STARTY: " + startY);
    ...
    {    	
		...
		if(spawn)
		{
			...
			if(!((BO3)Start.getObject()).getSettings().CanSpawnOnWater)
			{
				...
				//if(Start.y - 1 > TerrainControl.WORLD_DEPTH && Start.y - 1 < TerrainControl.WORLD_HEIGHT && world.getMaterial(Start.getX() + 8, Start.y - 1, Start.getZ() + 7).isLiquid())
				if(Start.y - 1 > TerrainControl.WORLD_DEPTH && Start.y - 1 < TerrainControl.WORLD_HEIGHT && world.getMaterial(Start.getX() + 8, highestBlocky, Start.getZ() + 7, IsOTGPlus).isLiquid())
				{
					return;
				}
			}
			...
			try
			{
				...
				TerrainControl.log(LogMarker.ERROR, "An unknown error occurred while calculating branches for BO3 " + Start.BO3Name + ". This is probably an error in the BO3's branch configuration, not a bug. If you can track this down, please tell me what caused it!");
				throw new RuntimeException();
			}
			...
			for(ChunkCoordinate chunkCoord : ObjectsToSpawn.keySet())
			{
				...
				//{
					//TerrainControl.log(LogMarker.INFO, "Initialised " + world.getPregeneratorInitialised() + " Running " + world.getPregeneratorIsRunning() +  " L" + world.getPregeneratedBorderLeft() + " R" + world.getPregeneratedBorderRight() + " T" + world.getPregeneratedBorderTop() + " B" + world.getPregeneratedBorderBottom());
					//TerrainControl.log(LogMarker.INFO, "Error at Chunk X" + chunkCoord.getChunkX() + " Z" + chunkCoord.getChunkZ() + ". " + (!World.getStructureCache().structureCache.containsKey(chunkCoord) ? (world.IsInsidePregeneratedRegion(chunkCoord, true) ? "Inside pregenned region" : "Not plotted") : World.getStructureCache().structureCache.get(chunkCoord) == null ? "Plotted and spawned" : World.getStructureCache().structureCache.get(chunkCoord).Start != null ? World.getStructureCache().structureCache.get(chunkCoord).Start.BO3Name : "Trees"));
					//throw new NotImplementedException();
				//} else {
					//TerrainControl.log(LogMarker.INFO, "Added BO3 at Chunk X" + chunkCoord.getChunkX() + " Z" + chunkCoord.getChunkZ() + ". " + this.Start.BO3Name);
				//}
				...
				//TerrainControl.log(LogMarker.INFO, "Added X" + chunkCoord.getChunkX() + " Z" + chunkCoord.getChunkZ() + " " + structureString);
			}	
			...
			for(ChunkCoordinate chunkCoord : SmoothingAreasToSpawn.keySet())
			{
				...
				//{
					...
					//throw new NotImplementedException();
				//} else {
					//TerrainControl.log(LogMarker.INFO, "Added BO3 smoothing area at Chunk X" + chunkCoord.getChunkX() + " Z" + chunkCoord.getChunkZ() + ". " + this.Start.BO3Name);
				//}
				...
				//{					
					//throw new NotImplementedException();
				//}
				//TerrainControl.log(LogMarker.INFO, "Added X" + chunkCoord.getChunkX() + " Z" + chunkCoord.getChunkZ() + " smoothing area");
			}			
			...
			if(ObjectsToSpawn.size() > 0)
			{
				...
				if(TerrainControl.getPluginConfig().SpawnLog)
				{
					...
					TerrainControl.log(LogMarker.DEBUG, Start.getObject().getName() + " " + totalBO3sSpawned + " object(s) plotted in " + (System.currentTimeMillis() - startTime) + " Ms, " + (branchesTried + 1) + " object(s) tried.");
				}
			}
		}
    }
    ...
    {
    	//TerrainControl.log(LogMarker.INFO, "GetMinimumSize");
    	...
    	//TerrainControl.log(LogMarker.INFO, Start.getObject(World.getName()).getName() + " minimum size in chunks X " + (Math.abs(left.getChunkX() - right.getChunkX()) + (smoothingRadiusInChunks * 2) + 1) + " Z " + (Math.abs(top.getChunkZ() - bottom.getChunkZ()) + (smoothingRadiusInChunks * 2) + 1));
    	
    	if(TerrainControl.getPluginConfig().SpawnLog)
    	{
    		//TerrainControl.log(LogMarker.INFO, Start.getObject(World.getName()).getName() + " minimum size calculated: "  + ObjectsToSpawn.size());
    		TerrainControl.log(LogMarker.DEBUG, "");
        	TerrainControl.log(LogMarker.DEBUG, Start.getObject().getName() + " minimum size: Width " + ((Integer)returnValue[1] + (Integer)returnValue[3] + 1) + " Length " + ((Integer)returnValue[0] + (Integer)returnValue[2] + 1) + " top " + (Integer)returnValue[0] + " right " + (Integer)returnValue[1] + " bottom " + (Integer)returnValue[2] + " left " + (Integer)returnValue[3]);
    	}
    ...
    */
    ...
    {  	    
    	...
    	public BranchDataItem(LocalWorld world, Random random, BranchDataItem parent, CustomObjectCoordinate branch, String startBO3Name, int currentDepth, int maxDepth, boolean minimumSize)
    	{
    		...
    		ChunkCoordinate = com.khorn.terraincontrol.util.ChunkCoordinate.fromBlockCoords(Branch.getX(), Branch.getZ());
    ...
    {    	    
    	if(TerrainControl.getPluginConfig().SpawnLog)
    	{
	    	...
	    	TerrainControl.log(LogMarker.TRACE, "");
	    	TerrainControl.log(LogMarker.TRACE, "-------- CalculateBranches " + Start.BO3Name + sminimumSize +" --------");
    	}
        ...
        {
        	TerrainControl.log(LogMarker.TRACE, "");
	        TerrainControl.log(LogMarker.TRACE, "---- Cycle 0 ----");
	        TerrainControl.log(LogMarker.TRACE, "Plotted X" + branchData.ChunkCoordinate.getChunkX() + " Z" + branchData.ChunkCoordinate.getChunkZ() + " - " + branchData.Branch.getObject().getName());
        }
    	...
    	while(!ProcessingDone)
    	{
    		...
    		{
    			TerrainControl.log(LogMarker.TRACE, "");
    			TerrainControl.log(LogMarker.TRACE, "---- Cycle " + Cycle + " ----");			
    		}
    		...
    		if(!SpawnedRequiredBranchesThisCycle)
    		{
    			if(TerrainControl.getPluginConfig().SpawnLog)
    			{
    				TerrainControl.log(LogMarker.TRACE, "All required branches done, plotting optional branches");
    			}
    		...
    		if(branchData.CannotSpawn)
    		{
    			if(minimumSize)
    			{
    				TerrainControl.log(LogMarker.INFO, "Error: Branching BO3 " + Start.BO3Name + " could not be spawned in minimum configuration (isRequiredBranch branches only).");
        ...
        for(BranchDataItem branchToAdd : AllBranchesBranchData)
        {
        	if(!branchToAdd.CannotSpawn)
        	{
        		...
        		if(TerrainControl.getPluginConfig().SpawnLog)
        		{
        			...
        			//TerrainControl.log(LogMarker.INFO, "CannotSpawn " + branchToAdd.Branch.BO3Name + ":" + branchToAdd.Branch.getRotation() + " X" + branchToAdd.ChunkCoordinate.getChunkX() + " Z" + branchToAdd.ChunkCoordinate.getChunkZ() + (minimumSize ? "" : " Y" + (branchToAdd.Branch.getY() - 1)) + " " + allParentsString);
	        		TerrainControl.log(LogMarker.INFO, "CannotSpawn " + branchToAdd.Branch.BO3Name + ":" + branchToAdd.Branch.getRotation() + " X" + branchToAdd.ChunkCoordinate.getChunkX() + " Z" + branchToAdd.ChunkCoordinate.getChunkZ() + (minimumSize ? "" : " Y" + (branchToAdd.Branch.getY())) + " " + allParentsString);
        		}
        		*/	         		
        	}
        }
    ...
    private void TraverseAndSpawnChildBranches(BranchDataItem branchData, boolean minimumSize)
    {
		...
		if(TerrainControl.getPluginConfig().SpawnLog)
		{
			...
			//TerrainControl.log(LogMarker.INFO, "TraverseAndSpawnChildBranches " + branchData.Branch.BO3Name + ":" + branchData.Branch.GetRotation() + " X" + branchData.ChunkCoordinate.getChunkX() + " Z" + branchData.ChunkCoordinate.getChunkZ() + (minimumSize ? "" : " Y" + (branchData.Branch.getY() - 1)) + " " + allParentsString);
			 * TerrainControl.log(LogMarker.INFO, "TraverseAndSpawnChildBranches " + branchData.Branch.BO3Name + ":" + branchData.Branch.GetRotation() + " X" + branchData.ChunkCoordinate.getChunkX() + " Z" + branchData.ChunkCoordinate.getChunkZ() + (minimumSize ? "" : " Y" + (branchData.Branch.getY())) + " " + allParentsString);
		}
		*/
    	if(!branchData.DoneSpawning)
    	{
    		//TerrainControl.log(LogMarker.INFO, "AddBranches " + branchData.Branch.getObject(World.getName()).getName());
    		...
    		if(!branchData.CannotSpawn)
    		{
    			for(BranchDataItem branchDataItem2 : branchData.getChildren(false))
    			{
    				...
    				{
    					//TerrainControl.log(LogMarker.INFO, "TraverseAndSpawnChildBranches2 " + branchDataItem2.Branch.getObject(World.getName()).getName());
	        ...
	        {	        	
	        	...
	        	{	        		
        			...
        			if(bo3 == null || bo3.isInvalidConfig)
        			{
		        		...
		        		if(bo3 == null)
		        		{
		        			TerrainControl.log(LogMarker.ERROR, "Error: Could not find BO3 file: " + childBranchDataItem.Branch.BO3Name + ".BO3 which is a branch of " + branchDataItem.Branch.BO3Name + ".BO3");
		        		}
        			}
	    	    			...
	    	    			if(childBranchDataItem.Branch.isRequiredBranch)// || ((BO3)childBranchDataItem.Branch.getObject(World.getName())).settings.canOverride)
	    	    			{
	    		    			...
	    		    			for(BranchDataItem branchDataItem3 : branchDataItem.getChildren(false))
	    		    			{
	    		    				if(!((BO3)branchDataItem3.Branch.getObject()).isInvalidConfig)
	    		    				{
		    		    				...
		    		    				{
		    		    					TerrainControl.log(LogMarker.INFO, "Error in BO3 file " + branchDataItem.Branch.BO3Name + ": multiple required branches at the same location");	    		    				
		    		    				}
	    		    				} else {
	    		    					TerrainControl.log(LogMarker.INFO, "Error in branches for BO3 " + Start.BO3Name + ". Branch " + branchDataItem3.Branch.BO3Name + "  was not a valid BO3 file ");
	    		    				}
	    		    			}
	    		    			if(!hasOnlyRequiredBranches)
	    		    			{
			   									...
			   									if(branchName.equals(mustBeInsideBO3Name.trim()))
			   									{
			   										...
			   										if(mustBeInsideBO3Rotation == null || rotation == Rotation.FromString(mustBeInsideBO3Rotation).getRotationId())			   										
			   										{
					   	   	    						...
					   	   	    						{				   	   	    							
				    	   	     	        				if(TerrainControl.getPluginConfig().SpawnLog)
				    	   	    	        				{
				    	   	     	        					TerrainControl.log(LogMarker.TRACE, "CannotBeInside branch " + childBranchDataItem.Branch.BO3Name + " was blocked by " + branchDataItem3.Branch.BO3Name);
				    	   	    	        				}
        		    	    		...
        		    	    		if(branches != null)
        		    	    		{
        		    	    			for(BranchDataItem a : branches)
        		    	    			{
        		    	    				...
        		    	    				if(a.Branch.BO3Name.equals(childBranchDataItem.Branch.BO3Name))
        		    	    				{
        		    	    					...
        		    	    					//TerrainControl.log(LogMarker.INFO, "A: " + a.Branch.BO3Name + " B: " + childBranchDataItem.Branch.BO3Name);
	        			...
	        			{        				
	        				...
	        				{		        				
		        				...
		        				{		        					
	    							...
	    							//TerrainControl.log(LogMarker.INFO, "collided with: " + collidingObject.BO3Name);
		        	...
		        	{		        		
		        		if(TerrainControl.getPluginConfig().SpawnLog)
		        		{
			        		...
			        		TerrainControl.log(LogMarker.TRACE, "Plotted " + childBranchDataItem.Branch.BO3Name + ":" + childBranchDataItem.Branch.getRotation() + " X" + childBranchDataItem.ChunkCoordinate.getChunkX() + " Z" + childBranchDataItem.ChunkCoordinate.getChunkZ() + (minimumSize ? "" : " Y" + (childBranchDataItem.Branch.getY())) + " " + allParentsString);
		        		}	        		
		        		...
		        		if(!childBranchDataItem.DoneSpawning && !childBranchDataItem.CannotSpawn)
		        		{
			        		...
			        		if(!collidedWithParentOrSibling && (!wasntBelowOther || !SpawnedBranchLastCycle) && childBranchDataItem.Branch.isRequiredBranch)
			        		{
			            		...
			            		if(TerrainControl.getPluginConfig().SpawnLog)
			            		{
			        	    		...
			        	    		TerrainControl.log(LogMarker.TRACE, "Rolling back " + branchDataItem.Branch.BO3Name + ":" + branchDataItem.Branch.getRotation() + " X" + branchDataItem.Branch.getChunkX() + " Z" + branchDataItem.Branch.getChunkZ() + " Y" + branchDataItem.Branch.getY() + " " + allParentsString + " because required branch "+ childBranchDataItem.Branch.BO3Name + " couldn't spawn. Reason: " + reason);
			            		}
    ...
    {    	
    	...
    	if(AllBranchesBranchDataHash.contains(branchData.branchNumber))
    	{
    		if(TerrainControl.getPluginConfig().SpawnLog)
    		{
	    		...
	    		TerrainControl.log(LogMarker.TRACE, "Deleted " + branchData.Branch.BO3Name + ":" + branchData.Branch.getRotation() + " X" + branchData.Branch.getChunkX() + " Z" + branchData.Branch.getChunkZ() + " Y" + branchData.Branch.getY() + " " + allParentsString);
    		}
		...
		if(branchData.Parent != null)
		{
    		...
    		{
    			//TerrainControl.log(LogMarker.INFO, "RollBackBranch 4: " + branchData.Parent.Branch.BO3Name + " <> " + branchData.Branch.BO3Name);
        ...
        {  
        	...
        	if(AllBranchesBranchDataHash.contains(branchDataItem.branchNumber))
        	{
        		if(TerrainControl.getPluginConfig().SpawnLog)
        		{
	        		...
	        		TerrainControl.log(LogMarker.TRACE, "Deleted " + branchDataItem.Branch.BO3Name + ":" + branchDataItem.Branch.getRotation() + " X" + branchDataItem.Branch.getChunkX() + " Z" + branchDataItem.Branch.getChunkZ() + " Y" + branchDataItem.Branch.getY() + " " + allParentsString);
        		}
    ...
    */
    ...
    {
    	//TerrainControl.log(LogMarker.INFO, "AddToChunk X" + chunkCoordinate.getChunkX() + " Z" + chunkCoordinate.getChunkZ());
    ...
    */
    ...
    {    	
    	//TerrainControl.log(LogMarker.INFO, "SpawnForChunk X" + chunkCoordinate.getChunkX() + " Z" + chunkCoordinate.getChunkZ() + " " + Start.BO3Name);
        ...
        {       
            ...
            for (CustomObjectCoordinate coordObject : objectsInChunk)
            {
                ...
                {
                	TerrainControl.log(LogMarker.FATAL, "Could not spawn chunk " + coordObject.BO3Name + " for structure " + Start.getObject().getName());
                	throw new RuntimeException();
                }
            }
    ...
    */
    ...
    {     	
        ...
        if(smoothingAreaInChunk != null && smoothingAreaInChunk.getValue() != null)
        {
			...
			{
				try {
					LocalMaterialData material = TerrainControl.readMaterial(((BO3)Start.getObject()).getSettings().smoothingSurfaceBlock);
			...
			if(((BO3)Start.getObject()).getSettings().smoothingGroundBlock != null && ((BO3)Start.getObject()).getSettings().smoothingGroundBlock.trim().length() > 0)
			{
				try
				{
					LocalMaterialData material = TerrainControl.readMaterial(((BO3)Start.getObject()).getSettings().smoothingGroundBlock);
					groundBlockSet = true;
			...
			if(((BO3)Start.getObject()).getSettings().replaceAbove != null && ((BO3)Start.getObject()).getSettings().replaceAbove.trim().length() > 0)
			{
				try
				{
					LocalMaterialData material = TerrainControl.readMaterial(((BO3)Start.getObject()).getSettings().replaceAbove);
			...
			if(((BO3)Start.getObject()).getSettings().replaceBelow != null && ((BO3)Start.getObject()).getSettings().replaceBelow.trim().length() > 0)
			{
				try
				{
					LocalMaterialData material = TerrainControl.readMaterial(((BO3)Start.getObject()).getSettings().replaceBelow);
            ...
            {            	
            	...
            	if(!originalTopBlocks.containsKey(ChunkCoordinate.fromChunkCoords(blockToSpawn.x, blockToSpawn.z)))
            	{
        			...
        			if(highestBlockY > TerrainControl.WORLD_DEPTH)
        			{
				...
				if(!goingUp)
				{
					...
					for(int y = yStart; y > yEnd; y--)
					{
	                    ...
	                    {	                    	
	                        ...
	                        blockToQueueForSpawn.material = TerrainControl.toLocalMaterialData(materialToSet,blockDataToSet);
        ...
        {       	
            ...
            {            	
	        	...
	        	if(diagonalLineFinalDestinationPointY == -1)
	        	{
	        		...
	        		//TerrainControl.log(LogMarker.INFO, "A smoothing area tried to spawn at Y -1 for structure " + Start.BO3Name + ". If you are creating empty chunks intentionally (for a sky-world for instance) you may wish to disable smoothing areas in your BO3s!");
	        		// Something is wrong!
	        		//throw new RuntimeException();
                ...
                {	 
                    ...
                    {                    	
                        ...
                        if(!dontAdd)
                        {
	                        ...
	                        {
	            	    		try {
	            					World.setBlock(originPointX, originPointY, originPointZ,TerrainControl.readMaterial("STONE"), null, true);
	            				} catch (InvalidConfigException e) {
    ...
    private void addToSpawnList(CustomObjectCoordinate coordObject, CustomObject parent)
    {
        ...
        if(chunkCoordinate != null)
        {
    		...
    		if(TerrainControl.getPluginConfig().SpawnLog)
    		{
	    		TerrainControl.log(LogMarker.WARN, "Error reading branch in BO3 " + parent.getName()  + " Could not find BO3: " + coordObject.BO3Name);
    		}
        }
    }
