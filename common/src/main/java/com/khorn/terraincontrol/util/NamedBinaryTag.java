...
public class NamedBinaryTag
{
     ...
     */
    public NamedBinaryTag findTagByName(String name)
    {
     ...
     */
    public NamedBinaryTag findNextTagByName(String name, NamedBinaryTag found)
    {
        if (type != Type.TAG_List && type != Type.TAG_Compound)
            return null;
        NamedBinaryTag[] subtags = (NamedBinaryTag[]) value;
        for (NamedBinaryTag subtag : subtags)
        {
            ...
            {
                NamedBinaryTag newFound = subtag.findTagByName(name);
                if (newFound != null)
                    if (newFound != found)
                    {
                        return newFound;
