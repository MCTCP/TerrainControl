...
public class CustomObjectCollection {
	...
	private CustomObject loadObject(File file, String worldName) {
		...
		if (file.isFile()) {
			...
			if (index != -1) {
				...
				// Get the object
				CustomObjectLoader loader = OTG.getCustomObjectManager().getObjectLoaders()
						.get(objectType.toLowerCase());
				if (loader != null) {
	...
	public CustomObject getObjectByName(String name, String worldName) {
		...
		// Only check the GlobalObjects if the WorldObjects directory has already been
		// searched
		...
		if (!customObjectFilesPerWorld.containsKey(worldName)) {
			...
			if (worldName != null && new File(
					OTG.getEngine().getOTGRootFolder() + File.separator + PluginStandardValues.PresetsDirectoryName
							+ File.separator + worldName + File.separator + "WorldObjects").exists()) {
				indexAllCustomObjectFilesInDir(new File(
						OTG.getEngine().getOTGRootFolder() + File.separator + PluginStandardValues.PresetsDirectoryName
								+ File.separator + worldName + File.separator + "WorldObjects"),
		...
		if (OTG.getPluginConfig().spawnLog) {
			OTG.log(LogMarker.WARN,
					"Could not find BO2/BO3 " + name + " in GlobalObjects "
							+ (worldName != null ? "and WorldObjects" : "") + " directory "
							+ (worldName != null ? "for world " + worldName : "") + ".");
		}
			...
			if (searchDir.isDirectory()) {
				for (File fileInDir : searchDir.listFiles()) {
					if (fileInDir.isDirectory()) {
						...
						if (fileInDir.getName().toLowerCase().endsWith(".bo4data")
								|| fileInDir.getName().toLowerCase().endsWith(".bo4")
								|| fileInDir.getName().toLowerCase().endsWith(".bo3")
								|| fileInDir.getName().toLowerCase().endsWith(".bo2")) {
							if (fileInDir.getName().toLowerCase().endsWith(".bo4data") || !customObjectFiles
									.containsKey(fileInDir.getName().toLowerCase().replace(".bo4data", "")
											.replace(".bo4", "").replace(".bo3", "").replace(".bo2", ""))) {
								customObjectFiles.put(fileInDir.getName().toLowerCase().replace(".bo4data", "")
										.replace(".bo4", "").replace(".bo3", "").replace(".bo2", ""), fileInDir);
							} else {
								if (OTG.getPluginConfig().spawnLog) {
				...
				if (searchDir.getName().toLowerCase().endsWith(".bo4data")
						|| searchDir.getName().toLowerCase().endsWith(".bo4")
						|| searchDir.getName().toLowerCase().endsWith(".bo3")
						|| searchDir.getName().toLowerCase().endsWith(".bo2")) {
					if (searchDir.getName().toLowerCase().endsWith(".bo4data")
							|| !customObjectFiles.containsKey(searchDir.getName().toLowerCase().replace(".bo4", "")
									.replace(".bo3", "").replace(".bo2", ""))) {
						customObjectFiles.put(searchDir.getName().toLowerCase().replace(".bo4data", "")
								.replace(".bo4", "").replace(".bo3", "").replace(".bo2", ""), searchDir);
					} else {
						if (OTG.getPluginConfig().spawnLog) {
