...
{
	public IcebergResource(IBiomeConfig biomeConfig, List<String> args, ILogger logger, IMaterialReader materialReader)
	...
	{
		int x = world.getDecorationArea().getChunkBeingDecoratedCenterX();
		int z = world.getDecorationArea().getChunkBeingDecoratedCenterZ();
			...
			int x, int y, int z, boolean p_205184_6_, int p_205184_7_, double p_205184_8_, int p_205184_10_)
	{
		...
		double lvt_16_1_ = p_205184_6_ ? (p_205184_8_ + 1.5707963267948966D)
				: (random.nextDouble() * 2.0D * Math.PI);

		for (int lvt_18_1_ = 0; lvt_18_1_ < p_205184_4_ - 3; 
				++lvt_18_1_)
		{
	...
	private void carve(int p_205174_1_, int p_205174_2_, int x, int y, int z, IWorldGenRegion world,
			boolean p_205174_5_, double p_205174_6_, int x3, int z3, int p_205174_9_, int p_205174_10_)
	{
		...
		for (int lvt_13_1_ = -lvt_11_1_; lvt_13_1_ < lvt_11_1_; lvt_13_1_++)
		{
			...
			{
				double lvt_15_1_ = signedDistanceEllipse(lvt_13_1_, lvt_14_1_, x3, z3, lvt_11_1_, lvt_12_1_,
						p_205174_6_);
				if (lvt_15_1_ < 0.0D)
				{
					...
					y2 = y + p_205174_2_;
					z2 = z + lvt_14_1_;
					LocalMaterialData material = world.getMaterialWithoutLoading(x2, y2, z2);
					if (isIcebergBlock(material) || material.isMaterial(LocalMaterials.SNOW_BLOCK))
						if (p_205174_5_)
						{
							world.setBlock(x2, y2, z2, LocalMaterials.WATER);
						} else
						{
							world.setBlock(x2, y2, z2, LocalMaterials.AIR);
	...
	{
		if (world.getMaterialWithoutLoading(x, y + 1, z).isMaterial(LocalMaterials.SNOW))
		{
			world.setBlock(x, y + 1, z, LocalMaterials.AIR);
		}
	}

	private void generateIcebergBlock(IWorldGenRegion p_205181_1_, Random p_205181_2_, int x, int y, int z,
			int p_205181_4_, int p_205181_5_, int p_205181_6_, int p_205181_7_, int p_205181_8_, int p_205181_9_,
			boolean p_205181_10_, int p_205181_11_, double p_205181_12_, boolean p_205181_14_,
			LocalMaterialData material)
	{
		double lvt_16_1_ = p_205181_10_
				? signedDistanceEllipse(p_205181_5_, p_205181_7_, 0, 0, p_205181_9_,
						getEllipseC(p_205181_6_, p_205181_4_, p_205181_11_), p_205181_12_)
		...
		if (lvt_16_1_ < 0.0D)
		{
			...
			if (lvt_16_1_ > lvt_19_1_ && p_205181_2_.nextDouble() > 0.9D)
				...
				return;
			setIcebergBlock(x2, y2, z2, p_205181_1_, p_205181_2_, p_205181_4_ - p_205181_6_, p_205181_4_, p_205181_10_,
					p_205181_14_, material);
		}
	}

	private void setIcebergBlock(int x, int y, int z, IWorldGenRegion world, Random p_205175_3_, int p_205175_4_,
			int p_205175_5_, boolean p_205175_6_, boolean p_205175_7_, LocalMaterialData material)
	{
		LocalMaterialData current = world.getMaterialWithoutLoading(x, y, z);
		if (current.isAir() || current.isMaterial(LocalMaterials.SNOW_BLOCK) || current.isMaterial(LocalMaterials.ICE)
				|| current.isMaterial(LocalMaterials.WATER))
		{
			...
			if (p_205175_7_ && !current.isMaterial(LocalMaterials.WATER)
					&& p_205175_4_ <= p_205175_3_.nextInt(Math.max(1, p_205175_5_ / lvt_11_1_)) + p_205175_5_ * 0.6D
					&& lvt_10_1_)
			{
				world.setBlock(x, y, z, LocalMaterials.SNOW_BLOCK);
			} else
			{
	...
	private double signedDistanceCircle(int p_205177_1_, int p_205177_2_, int x, int z, int p_205177_4_, Random random)
	{
		...
		return lvt_6_1_ + Math.pow((p_205177_1_ - x), 2.0D) + Math.pow((p_205177_2_ - z), 2.0D)
				- Math.pow(p_205177_4_, 2.0D);
	}

	private double signedDistanceEllipse(int p_205180_1_, int p_205180_2_, int x, int z, int p_205180_4_,
			int p_205180_5_, double p_205180_6_)
	{
		return Math
	...
	{
		return (material.isMaterial(LocalMaterials.PACKED_ICE) || material.isMaterial(LocalMaterials.SNOW_BLOCK)
				|| material.isMaterial(LocalMaterials.BLUE_ICE));
	}

	private boolean belowIsAir(IWorldGenRegion world, int x, int y, int z)
	{
		return (world.getMaterialWithoutLoading(x, y - 1, z).isAir());
	}

	private void smooth(IWorldGenRegion world, int x, int y, int z, int p_205186_3_, int p_205186_4_,
			boolean p_205186_5_, int p_205186_6_)
	{
			...
			for (int lvt_9_1_ = -lvt_7_1_; lvt_9_1_ <= lvt_7_1_; lvt_9_1_++)
			{
				for (int lvt_10_1_ = 0; lvt_10_1_ <= p_205186_4_; lvt_10_1_++)
				{
					...
					if (isIcebergBlock(material) || material.isMaterial(LocalMaterials.SNOW))
						...
						{
							world.setBlock(x2, y2, z2, LocalMaterials.AIR);
							world.setBlock(x2, y2 + 1, z2, LocalMaterials.AIR);

						} else if (isIcebergBlock(material))
						{

							LocalMaterialData[] lvt_13_1_ =
							{ world.getMaterialWithoutLoading(x2 + 1, y2, z2), world.getMaterialWithoutLoading(x2 - 1, y2, z2),
									world.getMaterialWithoutLoading(x2, y2, z2 + 1), world.getMaterialWithoutLoading(x2, y2, z2 - 1) };
							int lvt_14_1_ = 0;
