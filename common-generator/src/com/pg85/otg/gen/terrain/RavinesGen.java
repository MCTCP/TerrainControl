...
public class RavinesGen extends TerrainGenBase
{
    ...
    public RavinesGen(long worldSeed)
    {
        super(worldSeed);
    }

    private void placeBlocks(IWorldGenRegion worldGenRegion, long paramLong, ChunkBuffer generatingChunkBuffer, double paramDouble1, double paramDouble2, double paramDouble3, float paramFloat1, float paramFloat2, float paramFloat3, int size, double paramDouble4)
    {
        ...
        for (int j = 0; ; j++)
        {
            if (j >= worldGenRegion.getWorldConfig().getWorldHeightCap())
            {
                break;
            }
        ...
        for (int stepCount = 0; stepCount < size; stepCount++)
        {
            ...
            if (minDepth > worldGenRegion.getWorldConfig().getWorldHeightCap() - 8)
            {
                minDepth = worldGenRegion.getWorldConfig().getWorldHeightCap() - 8;
            }
            ...
            for (int localX = k; (i4 == 0) && (localX < m); localX++)
            {
                for (int localZ = i2; (i4 == 0) && (localZ < i3); localZ++)
                {
                    for (int localY = minDepth + 1; (i4 == 0) && (localY >= maxDepth - 1); localY--)
                    {
                        ...
                        if (localY < worldGenRegion.getWorldConfig().getWorldHeightCap())
                        {
    ...
    @Override
    protected void generateChunk(IWorldGenRegion worldGenRegion, ChunkCoordinate currentChunk, ChunkBuffer generatingChunkBuffer)
    {
        if (this.random.nextInt(100) >= worldGenRegion.getWorldConfig().getRavineRarity())
        {
            return;
        }
        
        double d1 = currentChunk.getBlockX() + this.random.nextInt(ChunkCoordinate.CHUNK_SIZE);
        double d2 = RandomHelper.numberInRange(random, worldGenRegion.getWorldConfig().getRavineMinAltitude(), worldGenRegion.getWorldConfig().getRavineMaxAltitude());
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            int size = RandomHelper.numberInRange(random, worldGenRegion.getWorldConfig().getRavineMinLength(), worldGenRegion.getWorldConfig().getRavineMaxLength());
            
            placeBlocks(worldGenRegion, this.random.nextLong(), generatingChunkBuffer, d1, d2, d3, f3, f1, f2, size, worldGenRegion.getWorldConfig().getRavineDepth());
        }
    }
}
