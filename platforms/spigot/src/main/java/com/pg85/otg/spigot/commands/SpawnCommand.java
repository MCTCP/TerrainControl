...
public class SpawnCommand
{
	public static boolean execute(CommandSender sender, HashMap<String, String> strings)
	{
		...
		if(objectToSpawn instanceof BO4)
		{
            ...
            int maxRadius = 1000;  
            OTG.getEngine().getLogger().log(LogMarker.INFO, LogCategory.PUBLIC, "Trying to plot BO4 structure within " + maxRadius + " chunks of player, with height bounds " + (force ? "disabled" : "enabled") + ". This may take a while."); 
                    ...
                    for (int z1 = playerZ - cycle; z1 <= playerZ + cycle; z1++)
                    {
                        if (x1 == playerX - cycle || x1 == playerX + cycle || z1 == playerZ - cycle || z1 == playerZ + cycle)
                        {
                            ...
                            if(!player.getWorld().isChunkGenerated(chunkCoord.getChunkX(), chunkCoord.getChunkZ()))
                            {
                            	...
                            	{
                                    OTG.getEngine().getLogger().log(LogMarker.INFO, LogCategory.PUBLIC, objectToSpawn.getName() + " was spawned at " + chunkCoordSpawned.getBlockX() + " ~ " + chunkCoordSpawned.getBlockZ());
            ...
            OTG.getEngine().getLogger().log(LogMarker.INFO, LogCategory.PUBLIC, objectToSpawn.getName() + " could not be spawned. This can happen if the world is currently generating chunks, if no biomes with enough space could be found, or if there is an error in the structure's files. Enable SpawnLog:true in OTG.ini and check the logs for more information.");
