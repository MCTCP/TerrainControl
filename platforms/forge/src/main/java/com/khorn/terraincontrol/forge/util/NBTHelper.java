...
public class NBTHelper
{
    ...
    private static NamedBinaryTag getNBTFromNMSTagList(NBTTagList nmsListTag)
    {
        ...
        NamedBinaryTag.Type listType = NamedBinaryTag.Type.values()[nmsListTag.tagAt(0).getId()];
        NamedBinaryTag listTag = new NamedBinaryTag(nmsListTag.getName(), listType);

        // Add all child tags
        for (int i = 0; i < nmsListTag.tagCount(); i++)
        {
            NBTBase nmsChildTag = nmsListTag.tagAt(i);
            switch (listType)
            {
                case TAG_End:
                    break;
    ...
    public static NBTTagCompound getNMSFromNBTTagCompound(NamedBinaryTag compoundTag)
    {
        ...
        for (NamedBinaryTag tag : childTags)
        {
            switch (tag.getType())
            {
                ...
                case TAG_String:
                case TAG_Int_Array:
                    nmsTag.setTag(tag.getName(), createTagNms(tag.getType(), tag.getName(), tag.getValue()));
                    break;
                case TAG_List:
