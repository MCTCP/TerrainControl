...
public class MapWriter implements Runnable {
	public static final int[] defaultColors = { 0x3333FF, 0x999900, 0xFFCC33, 0x333300, 0x00FF00, 0x007700, 0x99cc66,
			0x00CCCC, 0, 0, 0xFFFFFF, 0x66FFFF, 0xCCCCCC, 0xCC9966, 0xFF33cc, 0xff9999, 0xFFFF00, 0x996600, 0x009900,
			0x003300, 0x666600 };
	...
	public MapWriter(World mcWorld, int _size, Angle _angle, ICommandSender _sender, int _offsetX, int _offsetZ,
			String _label) {
		this.world = mcWorld;
		this.size = _size;
		this.sender = _sender;
	...
	private int[] getColors(World world2) {
		// OTG.log(LogMarker.DEBUG, "BukkitWorld::UUID:: {}", world.getWorldInfo().geti;
		LocalWorld bukkitWorld = OTG.getWorld(WorldHelper.getName(world2));
		if (bukkitWorld == null) {
			// OTG.log(LogMarker.ERROR, "BukkitWorld is null :: Make sure you add `{}` to
	...
	public void run() {
		...
		sender.sendMessage(new TextComponentString(
				BaseCommand.MESSAGE_COLOR + "You may notice a short performance impact during this process."));
		...
		for (int x = -height / 2; x < height / 2; x++) {
			for (int z = -width / 2; z < width / 2; z++) {
				// Forge currently generates the map all at once, so this isn't needed.
				 ...
				 * 
				 * if (time2 > time + 2000L) { sender.sendMessage(new TextComponentString(
				 * BaseCommand.MESSAGE_COLOR.toString() + ((x + height / 2) * 100 / height) +
				 * "%")); time = time2; }
				 */
					...
					for (int z1 = 0; z1 < 16; z1++) {
						...
						try {
							...
							OTG.log(LogMarker.DEBUG, "BiomeBuff Idx::{}<{}x/{}z>, Len::{}, ID::{} | Colors Len::{}",
									new Object[] { arrayPosition, x1, z1, biomeBuffer.length,
											OTGBiome.getIdForBiome(biomeBuffer[arrayPosition]), colors.length });
						}
					}
				}
			}
		...
		try {
			...
			sender.sendMessage(new TextComponentString(
					BaseCommand.ERROR_COLOR + "Exception while writing images: " + e.getLocalizedMessage()));
