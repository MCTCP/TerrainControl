...
public class KeyEntry implements IGuiListEntry {
	...
	public KeyEntry(OTGGuiDimensionSettingsList otgGuiDimensionSettingsList, SettingEntry<?> settingEntry,
			OTGGuiDimensionSettingsList parent, boolean isEnabled) {
		this(otgGuiDimensionSettingsList, settingEntry, parent);
		this.btnSettingEntry.enabled = isEnabled;
	}

	public KeyEntry(OTGGuiDimensionSettingsList otgGuiDimensionSettingsList, SettingEntry<?> settingEntry,
			OTGGuiDimensionSettingsList parent) {
	...
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
		...
		boolean showButtons = !(!isOverWorld && overWorldOnly) && !(gameIsRunning && !isNewConfig && newWorldOnly)
				&& (!isOverWorld || !this.settingEntry.name.equals("Preset"));

		int marginleft = x;
		this.otgGuiDimensionSettingsList.mc.fontRenderer.drawString(this.keyDesc, marginleft + 6,
				y + slotHeight / 2 - this.otgGuiDimensionSettingsList.mc.fontRenderer.FONT_HEIGHT / 2, 16777215);
		this.btnReset.x = marginleft + 240;
		this.btnReset.y = y;

		if (this.settingEntry.name.equals("Portal color") || this.settingEntry.name.equals("Allow cheats")
				|| this.settingEntry.name.equals("Bonus chest")) {
			this.btnReset.enabled = false;
		} else {
			this.btnReset.enabled = showButtons
					&& ((this.settingEntry.value == null && this.settingEntry.defaultValue != null)
							|| (this.settingEntry.value != null && ((this.settingEntry.valueType == ValueType.Bool
		...
		// Don't re-enable buttons that were disabled by other settings, like
		// AllowCheats and BonusChest.
		...
		if (this.settingEntry.name.equals("Game type") || this.settingEntry.name.equals("Portal color")
				|| this.settingEntry.name.equals("Preset") || this.settingEntry.valueType == ValueType.Bool) {
	...
	public void keyTyped(char typedChar, int keyCode)
	{
		if (
			this.settingEntry.valueType != ValueType.Bool && 
			!this.settingEntry.name.equals("Game type") && 
			!this.settingEntry.name.equals("Portal color") && 
			!this.settingEntry.name.equals("Preset")
		)
		{
			if (this.txtSettingsEntry.isFocused() || keyCode == 28 || keyCode == 83) // 28 + 83 is enter
			{
				...
				if (GuiScreen.isKeyComboCtrlA(keyCode) || GuiScreen.isKeyComboCtrlC(keyCode)
						|| GuiScreen.isKeyComboCtrlV(keyCode) || GuiScreen.isKeyComboCtrlX(keyCode) || keyCode == 14 || // backspace
						keyCode == 199 || keyCode == 203 || // left
						keyCode == 205 || // right
						keyCode == 207 || keyCode == 211 || keyCode == 28 || // 28 + 83 is enter
						keyCode == 83) {
					bIsFunctionkey = true;
				}
				...
				else if (this.settingEntry.valueType == ValueType.Int)
				{
					if (
						...
						(typedChar == '-' && this.txtSettingsEntry.getCursorPosition() == 0) || 
						bIsFunctionkey
					)
					{
						if (keyCode != 28 && keyCode != 83)
						...
						if (keyCode == 28 || keyCode == 83)
						{
							...
							if (this.settingEntry.name.equals("World border radius"))
							{
								...
								if (!this.parent.controlsScreen.selectedDimension.isNewConfig)
								{
									...
									forgeWorld = (ForgeWorld) ((ForgeEngine) OTG.getEngine()).getUnloadedWorld(
											this.parent.controlsScreen.selectedDimensionIndex == 0 ? "overworld"
													: this.parent.controlsScreen.selectedDimension.PresetName);
									if (forgeWorld == null)
									{
										forgeWorld = (ForgeWorld) ((ForgeEngine) OTG.getEngine()).getWorld(
												this.parent.controlsScreen.selectedDimensionIndex == 0 ? "overworld"
														: this.parent.controlsScreen.selectedDimension.PresetName);
									}
									...
									{
										ChunkCoordinate worldBorderCenterPoint = ChunkCoordinate.fromBlockCoords(
											forgeWorld.getWorld().getWorldInfo().getSpawnX(),
											forgeWorld.getWorld().getWorldInfo().getSpawnZ()
										);
										// 0 is disabled, 1 is 1 chunk, 2 is 3 chunks, 3 is 5 chunks etc
										double d2 = radius == 0 ? 6.0E7D : radius == 1 ? 16 : ((radius - 1) * 2 + 1) * 16;
										forgeWorld.getWorld().getWorldBorder().setCenter(
											worldBorderCenterPoint.getBlockX() + 8,
											worldBorderCenterPoint.getBlockZ() + 8
										);
				...
				else if (this.settingEntry.valueType == ValueType.Double)
				{
					if (
							...
							Character.isDigit(typedChar) || 
							(typedChar == '-' && this.txtSettingsEntry.getCursorPosition() == 0) || 
							(typedChar == '.' && !this.txtSettingsEntry.getText().contains("."))
					)
					{
						if (keyCode != 28 && keyCode != 83)
						{
	 ...
	 * Also called whenever the mouse is pressed on the parent screen, to make text
	 * boxes lose focus
	 */
	public boolean mousePressed(int slotIndex, int mouseX, int mouseY, int mouseEvent, int relativeX, int relativeY) {
		if (this.settingEntry.name.equals("Portal color")) {
			if (this.btnSettingEntry.mousePressed(this.otgGuiDimensionSettingsList.mc, mouseX, mouseY)) {
				...
				this.settingEntry.value = PortalColors.getNextFreePortalColor((String)this.settingEntry.value, this.parent.controlsScreen.dimensions, true);
				this.btnSettingEntry.displayString = this.settingEntry.getValueString();
				this.otgGuiDimensionSettingsList.applySettings();
				return true;
			}
		...
		if (this.settingEntry.name.equals("Game type")) {
			if (this.btnSettingEntry.mousePressed(this.otgGuiDimensionSettingsList.mc, mouseX, mouseY)) {
				...
				if (((String) this.settingEntry.value).toUpperCase().equals(GameType.SURVIVAL.toString())) {
					...
					for (IGuiListEntry entry : this.parent.getAllListEntries()) {
						if (entry.getLabelText().equals("Allow cheats") || entry.getLabelText().equals("Bonus chest")) {
							((KeyEntry) entry).btnSettingEntry.enabled = false;
							((KeyEntry) entry).settingEntry.value = false;
							((KeyEntry) entry).btnSettingEntry.displayString = ((KeyEntry) entry).settingEntry
									.getValueString();
						}
					}
			...
			if (this.btnSettingEntry.mousePressed(this.otgGuiDimensionSettingsList.mc, mouseX, mouseY)) {
				...
				this.otgGuiDimensionSettingsList.controlsScreen.selectPresetForDimensionMenu = new OTGGuiPresetList(
						this.otgGuiDimensionSettingsList.controlsScreen, true);
				this.otgGuiDimensionSettingsList.mc
						.displayGuiScreen(this.otgGuiDimensionSettingsList.controlsScreen.selectPresetForDimensionMenu);
			...
			if (this.txtSettingsEntry.mouseClicked(mouseX, mouseY, 0)) {
				...
				if (this.txtSettingsEntry.getText().length() == 0 && (this.settingEntry.valueType == ValueType.Int
						|| this.settingEntry.valueType == ValueType.Double)) {
					this.settingEntry.value = this.settingEntry.defaultValue;
					this.otgGuiDimensionSettingsList.applySettings();
				}
			}
		...
		if (this.btnReset.mousePressed(this.otgGuiDimensionSettingsList.mc, mouseX, mouseY)) {
			...
			if (this.settingEntry.name.equals("World border radius")) {
				...
				if (!this.parent.controlsScreen.selectedDimension.isNewConfig) {
					ForgeWorld forgeWorld = null;
					forgeWorld = (ForgeWorld) ((ForgeEngine) OTG.getEngine())
							.getUnloadedWorld(this.parent.controlsScreen.selectedDimensionIndex == 0 ? "overworld"
									: this.parent.controlsScreen.selectedDimension.PresetName);
					if (forgeWorld == null) {
						forgeWorld = (ForgeWorld) ((ForgeEngine) OTG.getEngine())
								.getWorld(this.parent.controlsScreen.selectedDimensionIndex == 0 ? "overworld"
										: this.parent.controlsScreen.selectedDimension.PresetName);
					}
					// ForgeWorld can be null for SP world creation menu
					if (Minecraft.getMinecraft().isSingleplayer() && forgeWorld != null
							&& forgeWorld.getWorldSession() != null) {
						ChunkCoordinate worldBorderCenterPoint = ChunkCoordinate.fromBlockCoords(
								forgeWorld.getWorld().getWorldInfo().getSpawnX(),
								forgeWorld.getWorld().getWorldInfo().getSpawnZ());
						// 0 is disabled, 1 is 1 chunk, 2 is 3 chunks, 3 is 5 chunks etc
						double d2 = radius == 0 ? 6.0E7D : radius == 1 ? 16 : ((radius - 1) * 2 + 1) * 16;
						forgeWorld.getWorld().getWorldBorder().setCenter(worldBorderCenterPoint.getBlockX() + 8,
								worldBorderCenterPoint.getBlockZ() + 8);
						forgeWorld.getWorld().getWorldBorder().setTransition(d2);
					}
				}
