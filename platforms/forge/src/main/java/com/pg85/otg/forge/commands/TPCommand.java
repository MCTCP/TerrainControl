...
public class TPCommand extends BaseCommand {
	...
	public boolean onCommand(ICommandSender sender, List<String> args) {
		...
		if (biomeOrDimensionName != null && biomeOrDimensionName.trim().length() > 0) {
			...
			sender.sendMessage(
					new TextComponentTranslation(MESSAGE_COLOR + "Searching for destination biome or dimension \""
							+ VALUE_COLOR + biomeOrDimensionName + MESSAGE_COLOR + "\"."));
									...
									if (z1 == playerZ - cycle || z1 == playerZ + cycle) {
										...
										if (biome != null && biome.getIds().getOTGBiomeId() == biomeId) {
											sender.sendMessage(new TextComponentTranslation(
													MESSAGE_COLOR + "Teleporting to \"" + VALUE_COLOR
															+ biomeOrDimensionName + MESSAGE_COLOR + "\"."));
			...
			sender.sendMessage(new TextComponentTranslation(
					ERROR_COLOR + "Could not find biome \"" + biomeOrDimensionName + "\"."));
			return true;
		}
		return false;
	}
