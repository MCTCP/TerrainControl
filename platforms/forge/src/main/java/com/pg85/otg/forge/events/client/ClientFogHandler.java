...
public class ClientFogHandler
{
    ...
    // Blend the fog color
    private Vec3d blendFogColors(World mcworld, EntityLivingBase entity, float red, float green, float blue, double renderPartialTicks)
    {
        ...
        float time = MathHelper.clamp(
                MathHelper.cos(
                        mcworld.getCelestialAngle((float) renderPartialTicks) * (float) Math.PI * 2.0F) * 2.0F + 0.5F,
                0, 1);
