...
public class OTGBiomeProvider extends BiomeProvider implements LayerSource
{
		...
		(instance) ->
				instance.group(
					Codec.LONG.fieldOf("seed").stable().forGetter((provider) ->
							provider.seed),
					Codec.BOOL.optionalFieldOf("legacy_biome_init_layer", Boolean.FALSE, Lifecycle.stable()).forGetter((provider) ->
							provider.legacyBiomeInitLayer),
					Codec.BOOL.fieldOf("large_biomes").orElse(false).stable().forGetter((provider) ->
							provider.largeBiomes),
	...
	{
		super(
			BIOMES.stream().map((p_242638_1_) ->
					() ->
							registry.func_243576_d(p_242638_1_))
		);
		this.seed = seed;
