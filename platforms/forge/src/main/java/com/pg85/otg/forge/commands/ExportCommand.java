...
public class ExportCommand extends BaseCommand {
	...
	public boolean onCommand(ICommandSender sender, List<String> args) {
		...
		sender.sendMessage(new TextComponentString(
				String.format("%sBO3 %s%s %2$s(%dx%dx%d) %1$shas been saved to GlobalObjects.", MESSAGE_COLOR,
						VALUE_COLOR, bo3Name, selection.getWidth(), selection.getHeight(), selection.getLength())));

		if (branch)
			sender.sendMessage(new TextComponentString(
					MESSAGE_COLOR + "BO3 was larger than 32x32 so it has been split into branches."));
