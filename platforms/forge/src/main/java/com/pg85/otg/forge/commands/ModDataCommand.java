	...
	public boolean onCommand(ICommandSender sender, List<String> args) {
		...
		if (args.size() == 2) {
			...
			FMLInterModComms.sendRuntimeMessage(OTGPlugin.Instance, args.get(1), "GetModData",
					sender.getEntityWorld().getWorldInfo().getWorldName() + "," + chunkCoord.getChunkX() + ","
			...
			try {
				...
				for (int x = -radius; x <= radius; x++) {
					for (int z = -radius; z <= radius; z++) {
						...
						FMLInterModComms.sendRuntimeMessage(OTGPlugin.Instance, args.get(1), "GetModData",
								sender.getEntityWorld().getWorldInfo().getWorldName() + "," + chunkCoord2.getChunkX()
										+ "," + chunkCoord2.getChunkZ());
					}
				}
